...
function CastSunRay( event )
	...
	-- Create particle FX
	local particleName = "particles/custom_particles/alakazam/alakazam_blast.vpcf"
...
function CheckForCollision( event )
	...
	--talent
	local talent = caster:FindAbilityByName("alakazam_blast_damage_bonus")

	if talent:GetLevel() > 0 then
		hpPercentDamage = hpPercentDamage + talent:GetSpecialValueFor("value")
	end
