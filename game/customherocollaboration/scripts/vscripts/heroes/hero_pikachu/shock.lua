...
function SearchArea(keys)
	...
	local shockdamage = ability:GetLevelSpecialValueFor("damage", (ability:GetLevel() -1))
	...
	--talent
	local talent = caster:FindAbilityByName("pikachushock_bonus_damage")

	if talent:GetLevel() > 0 then
		shockdamage = shockdamage + talent:GetSpecialValueFor("value")
	end
	...
	Timers:CreateTimer(delay, function ()
		...
		if target ~= nil then
			...
			ApplyDamage({victim = target, attacker = caster, damage = shockdamage, damage_type = ability:GetAbilityDamageType()})
